...
class AssistenteDigital:
    def __init__(self, root):
        ...
        self.root.geometry("550x650")  # Tamanho padrão da janela
        ...
        self.apply_custom_styles()  # Aplica o tema escuro

        # Define o ícone da janela (aparece só no topo da janela)
        ...
        self.root.configure(background=self.colors["background"])  # Fundo escuro

        # Carrega os ícones dos botões
        self.icons = {}
        self.load_icons()

        # Variáveis de controle do estado do assistente
        self.running = False
        self.thread = None
        ...
        title_label = ttk.Label(
            header_frame,
            text="Assistente de Produtividade",
            font=("Arial", 14, "bold"),
        ...
        status_text = ttk.Label(
            self.status_frame, text="Status:", font=("Arial", 10), style="TLabel"
        )
        ...
        # Ícones nos botões (tem que manter referência senão some)
        if "start" in self.icons:
            ...
            if not hasattr(self, "_button_images"):
                self._button_images = []
            self._button_images.append(self.icons["start"])
        if "stop" in self.icons:
            self.stop_button.configure(image=self.icons["stop"], compound=tk.LEFT)
            if not hasattr(self, "_button_images"):
                self._button_images = []
            self._button_images.append(self.icons["stop"])

        # Configurações principais
        config_frame = ttk.LabelFrame(
            self.main_frame, text=" Configurações ", padding=(15, 10)
        )
        ...
        ttk.Label(
            interval_frame, text="Intervalo entre atividades:", style="Card.TLabel"
        ).pack(side=tk.LEFT)
        ...
        ttk.Label(interval_frame, text="segundos", style="Card.TLabel").pack(
            side=tk.LEFT
        )
        ...
        ttk.Checkbutton(
            config_frame,
            text="Alternar Caps Lock",
            variable=self.use_caps_var,
            style="TCheckbutton",
        ).pack(anchor="w", pady=3)
        ttk.Checkbutton(
            config_frame,
            text="Simular pressionamento de teclas",
            variable=self.use_keys_var,
            style="TCheckbutton",
        ).pack(anchor="w", pady=3)

        # Config de teclas
        keys_config_frame = ttk.LabelFrame(
            self.main_frame, text=" Configurações de Teclas ", padding=(15, 10)
        )
        ...
        ttk.Label(
            keys_option_frame,
            text="Selecione as teclas a simular:",
            style="Card.TLabel",
        ...
        info_label = ttk.Label(
            footer_frame,
            text="Dica: Ajuste as configs conforme o ambiente.",
            font=("Arial", 9),
            style="Secondary.TLabel",
        )
        info_label.pack(side=tk.LEFT)

        # Fecha thread certinho ao fechar janela
        self.root.protocol("WM_DELETE_WINDOW", self.on_close)

        # Verifica se a thread tá rodando de tempos em tempos
    ...
    def activity_loop(self):
        ...
        while self.running:
            try:
                ...
                wait_time = max(
                    0.5, interval + variation
                )  # Permitir intervalo mínimo de 0.5 segundos
    ...
    def check_thread_active(self):
        ...
        if self.running:
            ...
            self.check_thread_id = self.root.after(
                self.check_thread_interval, self.check_thread_active
            )
